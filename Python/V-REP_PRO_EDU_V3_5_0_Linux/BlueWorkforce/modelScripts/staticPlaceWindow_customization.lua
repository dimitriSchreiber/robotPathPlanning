...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.OLDSTATICPLACEWINDOW_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.OLDSTATICPLACEWINDOW_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.OLDSTATICPLACEWINDOW_TAG,'')
    end
end
...
function createDlg()
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        comboSensor=sim.UI_populateCombobox(ui,23,sens,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.STATICPLACEWINDOW_SENSOR_REF)),true,{{simBWF.NONE_TEXT,-1}})

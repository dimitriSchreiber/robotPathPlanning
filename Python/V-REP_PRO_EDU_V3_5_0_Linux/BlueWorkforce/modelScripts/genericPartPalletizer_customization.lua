...
function readInfo()
    local data=sim.readCustomDataBlock(model,'XYZ_PARTPALLETIZER_INFO')
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,'XYZ_PARTPALLETIZER_INFO',sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,'XYZ_PARTPALLETIZER_INFO','')
    end
end
...
function conveyorChange_callback(ui,id,newIndex)
    ...
    simBWF.setReferencedObjectHandle(model,simBWF.PALLETIZER_CONVEYOR_REF,newLoc)
    simBWF.markUndoPoint()
end
...
function createDlg()
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        comboConveyor=sim.UI_populateCombobox(ui,10,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.PALLETIZER_CONVEYOR_REF)),true,{{simBWF.NONE_TEXT,-1}})
...
if (sim_call_type==sim.customizationscriptcall_initialization) then
    ...
    if _info['conveyor'] then
        simBWF.setReferencedObjectHandle(model,simBWF.PALLETIZER_CONVEYOR_REF,sim.getObjectHandle_noErrorNoSuffixAdjustment(_info['conveyor']))
        _info['conveyor']=nil
    end
    writeInfo(_info)

...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.OLDPARTREPO_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.OLDPARTREPO_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.OLDPARTREPO_TAG,'')
    end
end
    ...
    if #parts>0 then
        ...
        if prop['name']~=newVal and #newVal>0 then
            ...
            if allNames[newVal] then
                ...
                parts,partIndex=sim.UI_populateCombobox(ui1,4,partTable,nil,newVal..'   ('..sim.getObjectName(h)..')',true,nil)
            end
        end
        displayPartProperties()
    end
...
function createDlg1()
    if (not ui1) and simBWF.canOpenPropertyDialog() then
        ...
        parts,partIndex=sim.UI_populateCombobox(ui1,4,partTable,nil,previousItemName,true,nil)
...
if (sim_call_type==sim.customizationscriptcall_initialization) then
    ...
    local objs=sim.getObjectsWithTag(simBWF.OLDPARTREPO_TAG,true)
    if #objs>1 then
        sim.removeModel(model)
        sim.removeObjectFromSelection(sim.handle_all)
        objs=sim.getObjectsWithTag(simBWF.OLDPARTREPO_TAG,true)
...
function pricing_callback()
    if not pricingRequest then
        local objects={}
        local tags={simBWF.RAGNAR_TAG,simBWF.RAGNARGRIPPER_TAG,simBWF.OLDLOCATION_TAG,simBWF.TRACKINGWINDOW_TAG,"XYZ_STATICPICKWINDOW_INFO","XYZ_DETECTIONWINDOW_INFO",simBWF.CONVEYOR_TAG}
        for i=1,#tags,1 do

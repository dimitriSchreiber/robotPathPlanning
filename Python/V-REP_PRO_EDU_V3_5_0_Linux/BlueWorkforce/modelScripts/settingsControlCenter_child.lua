if (sim_call_type==sim.childscriptcall_initialization) then
    ...
    local data=sim.unpackTable(sim.readCustomDataBlock(model,simBWF.OLDOVERRIDE_TAG))
    if sim.boolAnd32(data['bitCoded'],8)>0 then
        if version>30303 then
            -- Check if some models are coincident
            -- We check only models with tags that start with 'XYZ_' or tags that are simBWF.RAGNAR_TAG or simBWF.CONVEYOR_TAG
            ...
            for i=1,#objects,1 do
                ...
                if sim.boolAnd32(p,sim.modelproperty_not_model)==0 then
                    ...
                    if tags then
                        for j=1,#tags,1 do
                            local tag=tags[j]
                            if string.find(tag,'XYZ_')==1 or tag==simBWF.RAGNAR_TAG or tag==simBWF.CONVEYOR_TAG then
                                if tag~=simBWF.PART_TAG and tag~='XYZ_PARTLABEL_INFO' then

...
if (sim_call_type==sim.childscriptcall_initialization) then
    ...
    local modelData=sim.readCustomDataBlock(model,simBWF.CONVEYOR_TAG)
...
if (sim_call_type==sim.childscriptcall_sensing) then
    ...
    local data=sim.readCustomDataBlock(trackingWindowModel,simBWF.TRACKINGWINDOW_TAG)
    ...
    if 1==smallestStage then -- full
        local dat=sim.readCustomDataBlock(model,simBWF.CONVEYOR_TAG)
        dat=sim.unpackTable(dat)
        dat['putCartridgeDown'][currentCartridgeIndex]=true
        sim.writeCustomDataBlock(model,simBWF.CONVEYOR_TAG,sim.packTable(dat))
    ...
    sim.writeCustomDataBlock(trackingWindowModel,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))

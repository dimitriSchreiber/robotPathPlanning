...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.SHIPPINGBOX_PART_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.SHIPPINGBOX_PART_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.SHIPPINGBOX_PART_TAG,'')
    end
end

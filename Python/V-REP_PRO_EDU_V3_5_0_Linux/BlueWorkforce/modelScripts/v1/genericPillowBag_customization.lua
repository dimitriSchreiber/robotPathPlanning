...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.PILLOWBAG_PART_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.PILLOWBAG_PART_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.PILLOWBAG_PART_TAG,'')
    end
end
...
function updateModel()
    ...
    if sim.boolAnd32(bitC,8)>0 then
        ...
        sim.writeCustomDataBlock(h,simBWF.LABEL_PART_TAG,sim.packTable(labelData))
    end
end

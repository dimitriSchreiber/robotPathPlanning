...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.PARTTAGGER_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.PARTTAGGER_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.PARTTAGGER_TAG,'')
    end
end
...
function setDlgItemContent()
    if ui then
        ...
        simUI.setEditValue(ui,20,simBWF.format("%.0f , %.0f , %.0f",config.size[1]*1000,config.size[2]*1000,config.size[3]*1000),true)
...
function updateEnabledDisabledItemsDlg()
    if ui then
        ...
        simUI.setEnabled(ui,1365,enabled,true)

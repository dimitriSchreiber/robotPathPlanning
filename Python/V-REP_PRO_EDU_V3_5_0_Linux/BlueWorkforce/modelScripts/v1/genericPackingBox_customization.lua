...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.PACKINGBOX_PART_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.PACKINGBOX_PART_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.PACKINGBOX_PART_TAG,'')
    end
end
    ...
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            ...
            <group layout="form" flat="false">
                ...
                <checkbox text="Length (%)" on-change="lidA_callback" id="10" />
            ...
            <group layout="form" flat="false">
                ...
                <checkbox text="Length (%)" on-change="lidB_callback" id="13" />

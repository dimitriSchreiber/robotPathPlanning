...
function updateStartStopTriggerComboboxes()
    ...
    comboStopTrigger=sim.UI_populateCombobox(ui,200,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.CONVEYOR_STOP_SIGNAL_REF)),true,{{simBWF.NONE_TEXT,-1}})
    comboStartTrigger=sim.UI_populateCombobox(ui,201,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.CONVEYOR_START_SIGNAL_REF)),true,{{simBWF.NONE_TEXT,-1}})
end

function updateMasterCombobox()
    local c=readInfo()
    local loc=getAvailableMasterConveyors()
    comboMaster=sim.UI_populateCombobox(ui,202,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.CONVEYOR_MASTER_CONVEYOR_REF)),true,{{simBWF.NONE_TEXT,-1}})
end

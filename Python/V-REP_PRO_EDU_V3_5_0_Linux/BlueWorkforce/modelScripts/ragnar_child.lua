...
--        windowH=partTrackingWindowHandle
    ...
    if windowH>=0 then
        local data=sim.readCustomDataBlock(windowH,simBWF.TRACKINGWINDOW_TAG)
        data=sim.unpackTable(data)
        data['associatedRobotTrackingCorrectionTime']=avgPickTm
        sim.writeCustomDataBlock(windowH,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))
    end
end
...
function ragnar_endPlaceTime(isOtherLocation)
    ...
    if isOtherLocation then
        ...
        if locationTrackingWindowHandle>=0 then
            local data=sim.readCustomDataBlock(locationTrackingWindowHandle,simBWF.TRACKINGWINDOW_TAG)
            data=sim.unpackTable(data)
            data['associatedRobotTrackingCorrectionTime']=correctionT
            sim.writeCustomDataBlock(locationTrackingWindowHandle,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))
        end
    end
end
...
function getToolHandleAndStacking()
    ...
    if h>=0 then
        local data=sim.readCustomDataBlock(h,simBWF.RAGNARGRIPPER_TAG)
        if data then
            data=sim.unpackTable(data)
    ...
    for i=1,4,1 do
        ...
        if h>=0 then
            ...
            if data then
                ...
                data=sim.readCustomDataBlock(h,simBWF.OLDLOCATION_TAG)
                if data then
                    data=sim.unpackTable(data)
...
function ragnar_incrementTrackedLocationProcessingStage(trackingLocation)
    ...
    if wind>=0 then
        if lastPlaceWindowIndex==2 then
            local data=sim.readCustomDataBlock(wind,simBWF.OLDSTATICPLACEWINDOW_TAG)
            data=sim.unpackTable(data)
            ...
            sim.writeCustomDataBlock(wind,simBWF.OLDSTATICPLACEWINDOW_TAG,sim.packTable(data))
        else
            local data=sim.readCustomDataBlock(wind,simBWF.TRACKINGWINDOW_TAG)
            data=sim.unpackTable(data)
            local tbl=data['targetPositionsToMarkAsProcessed']
            tbl[#tbl+1]=trackingLocation['dummyHandle']
            data['targetPositionsToMarkAsProcessed']=tbl
            sim.writeCustomDataBlock(wind,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))
        end
    end
end
...
getAllTargetsInTrackingWindow=function(trackingWindowHandle,locationName,processingStage)
    ...
    if trackingWindowHandle>=0 then
        local data=sim.readCustomDataBlock(trackingWindowHandle,simBWF.TRACKINGWINDOW_TAG)
        data=sim.unpackTable(data)
...
removePartFromTrackingWindow=function(trackingWindowHandle,partHandle)
    if trackingWindowHandle>=0 then
        ...
        sim.writeCustomDataBlock(trackingWindowHandle,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))
    end
end
...
freezeTrackingWindow=function(windowHandle)
    if windowHandle>=0 then
        local data=sim.readCustomDataBlock(windowHandle,simBWF.TRACKINGWINDOW_TAG)
        data=sim.unpackTable(data)
        data['freezeStaticWindow']=true
        sim.writeCustomDataBlock(windowHandle,simBWF.TRACKINGWINDOW_TAG,sim.packTable(data))
    end
end
        ...
        if sim.isHandleValid(attachedParts[1])>0 then
            ...
            for i=2,#attachedParts,1 do
                ...
                if ragnar_getAttachToTarget() then
                    ...
                    local data=sim.readCustomDataBlock(attachedParts[i],simBWF.PART_TAG)
                    data=sim.unpackTable(data)
                    data['giveUpOwnership']=true
                    sim.writeCustomDataBlock(attachedParts[i],simBWF.PART_TAG,sim.packTable(data))
                else
...
attachPart1ToPart2=function(part1,part2)
    if (sim.isHandleValid(part1)>0) and (sim.isHandleValid(part2)>0) then
        ...
        local data=sim.unpackTable(sim.readCustomDataBlock(part1,simBWF.PART_TAG))
        data['giveUpOwnership']=true
        sim.writeCustomDataBlock(part1,simBWF.PART_TAG,sim.packTable(data))
...
updateMotionParameters=function()
    ...
    local ragnarSettings=sim.unpackTable(sim.readCustomDataBlock(model,simBWF.RAGNAR_TAG))
...
function sysCall_threadmain()
    ...
    local ragnarSettings=sim.readCustomDataBlock(model,simBWF.RAGNAR_TAG)

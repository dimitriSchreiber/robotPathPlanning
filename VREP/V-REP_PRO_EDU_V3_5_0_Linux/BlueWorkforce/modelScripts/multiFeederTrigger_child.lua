...
wasMultiFeederTriggered=function()
    local data=sim.unpackTable(sim.readCustomDataBlock(model,simBWF.MULTIFEEDER_TAG))
        ...
        if true then --feederAlgo then
            ...
            if feederToTrigger and feederToTrigger>=0 then
                ...
                if sim.isHandleValid(feederToTrigger)>0 then
                    local data=sim.readCustomDataBlock(feederToTrigger,simBWF.PARTFEEDER_TAG)
                    if data then
                        data=sim.unpackTable(data)
                        if sim.boolAnd32(data['bitCoded'],4+8+16)==16 then
                            data['multiFeederTriggerCnt']=data['multiFeederTriggerCnt']+1
                            sim.writeCustomDataBlock(feederToTrigger,simBWF.PARTFEEDER_TAG,sim.packTable(data))
                        end
                    else
                        data=sim.readCustomDataBlock(feederToTrigger,simBWF.MULTIFEEDER_TAG)
                        if data then
                            data=sim.unpackTable(data)
                            if sim.boolAnd32(data['bitCoded'],4+8+16)==16 then
                                data['multiFeederTriggerCnt']=data['multiFeederTriggerCnt']+1
                                sim.writeCustomDataBlock(feederToTrigger,simBWF.MULTIFEEDER_TAG,sim.packTable(data))
                            end
                        end
                    end
                end

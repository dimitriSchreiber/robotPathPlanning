...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.TRAY_PART_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.TRAY_PART_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.TRAY_PART_TAG,'')
    end
end
...
function setShapeMass(handle,m)
...
function updateTray()
    ...
    local c=simBWF.readPartInfo(model)
    c['name']='TRAY'

...
function sysCall_beforeDelete(data)
    local obj=simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_SENSOR_REF)
    if obj>=0 and data.objectHandles[obj] then
        updateAfterObjectDeletion=true
    end
    local obj=simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_CONVEYOR_REF)
    if obj>=0 and data.objectHandles[obj] then
        updateAfterObjectDeletion=true
    end
    local obj=simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STOPSIGNAL_REF)
    if obj>=0 and data.objectHandles[obj] then
        updateAfterObjectDeletion=true
    end
    local obj=simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STARTSIGNAL_REF)
    if obj>=0 and data.objectHandles[obj] then
        updateAfterObjectDeletion=true
    end
end
...
function getDefaultInfoForNonExistingFields(info)
    ...
    if not info['maxProductionCnt'] then
        info['maxProductionCnt']=0
    end
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.PARTFEEDER_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.PARTFEEDER_TAG,'')
    end
end
...
function maxProductionCntChange_callback(ui,id,newVal)
    local c=readInfo()
    local v=tonumber(newVal)
    if v then
        if v<0 then v=0 end
...
function sensorComboChange_callback(ui,id,newIndex)
    ...
    simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_SENSOR_REF,sens)
    simBWF.markUndoPoint()
end

function conveyorComboChange_callback(ui,id,newIndex)
    local conv=comboConveyor[newIndex+1][2]
    simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_CONVEYOR_REF,conv)
    simBWF.markUndoPoint()
end
...
function updateEnabledDisabledItemsDlg1()
    if ui1 then
        ...
        simUI.setEnabled(ui1,1365,enabled,true)
        ...
        simUI.setEnabled(ui1,63,enabled)
...
function refreshDlg()
    if ui1 then
        ...
        comboConveyor=sim.UI_populateCombobox(ui1,998,loc,{},simBWF.getObjectAltNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_CONVEYOR_REF)),true,{{simBWF.NONE_TEXT,-1}})
        loc=getAvailableSensors()
        comboSensor=sim.UI_populateCombobox(ui1,999,loc,{},simBWF.getObjectAltNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_SENSOR_REF)),true,{{simBWF.NONE_TEXT,-1}})

        updateStartStopTriggerComboboxes()
        
        simUI.setEditValue(ui1,1365,simBWF.getObjectAltName(model),true)
        simUI.setEditValue(ui1,60,simBWF.format("%.0f , %.0f , %.0f",config.size[1]*1000,config.size[2]*1000,config.size[3]*1000),true)
        ...
        if config['maxProductionCnt']==0 then
            simUI.setEditValue(ui1,63,"unlimited",true)
        else
            simUI.setEditValue(ui1,63,simBWF.format("%.0f",config['maxProductionCnt']),true)
        end
...
function triggerStopChange_callback(ui,id,newIndex)
    ...
    simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_STOPSIGNAL_REF,sens)
    if simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STARTSIGNAL_REF)==sens then
        simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_STARTSIGNAL_REF,-1)
    end
    simBWF.markUndoPoint()
    updateStartStopTriggerComboboxes()
end
...
function triggerStartChange_callback(ui,id,newIndex)
    ...
    simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_STARTSIGNAL_REF,sens)
    if simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STOPSIGNAL_REF)==sens then
        simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_STOPSIGNAL_REF,-1)
    end
    simBWF.markUndoPoint()
    updateStartStopTriggerComboboxes()
end
...
function updateStartStopTriggerComboboxes()
    ...
    comboStopTrigger=sim.UI_populateCombobox(ui1,100,loc,{},simBWF.getObjectAltNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STOPSIGNAL_REF)),true,{{simBWF.NONE_TEXT,-1}})
    comboStartTrigger=sim.UI_populateCombobox(ui1,101,loc,{},simBWF.getObjectAltNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.FEEDER_STARTSIGNAL_REF)),true,{{simBWF.NONE_TEXT,-1}})
end
...
function createDlg1()
    ...
    <tab title="General" layout="form">
               ...
               <label text="Enabled"/>
                ...
                <label text="Distribution stretch (X, Y, Z, in mm)"/>
                <edit on-editing-finished="sizeChange_callback" id="60"/>

                <radiobutton text="Time trigger (parts/s)" on-click="triggerTypeClick_callback" id="1000" />
                ...
                <radiobutton text="User defined trigger" on-click="triggerTypeClick_callback" id="1002" />
                <button text="Edit trigger algorithm"  on-click="dropAlgorithmClick_callback" id="3" />
                ...
                <label text="Max. production count"/>
                <edit on-editing-finished="maxProductionCntChange_callback" id="63"/>
...
function sysCall_init()
    ...
    if _info['sensor'] then
        simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_SENSOR_REF,sim.getObjectHandle_noErrorNoSuffixAdjustment(_info['sensor']))
        _info['sensor']=nil
    end
    if _info['conveyor'] then
        simBWF.setReferencedObjectHandle(model,simBWF.FEEDER_CONVEYOR_REF,sim.getObjectHandle_noErrorNoSuffixAdjustment(_info['conveyor']))
        _info['conveyor']=nil
    end
    ----------------------------------------
    writeInfo(_info)
...
function showOrHideUi1IfNeeded()

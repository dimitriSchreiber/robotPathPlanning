function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.OLDLOCATION_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.OLDLOCATION_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.OLDLOCATION_TAG,'')
    end
end

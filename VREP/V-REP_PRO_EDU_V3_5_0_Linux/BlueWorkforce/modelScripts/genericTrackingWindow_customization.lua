...
function createDlg()
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        comboConveyor=sim.UI_populateCombobox(ui,10,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.OLDTRACKINGWINDOW_CONVEYOR_REF)),true,{{simBWF.NONE_TEXT,-1}})
        loc=getAvailableDetectionOrTrackingWindows()
        comboInput=sim.UI_populateCombobox(ui,11,loc,{},simBWF.getObjectNameOrNone(simBWF.getReferencedObjectHandle(model,simBWF.OLDTRACKINGWINDOW_INPUT_REF)),true,{{simBWF.NONE_TEXT,-1}})

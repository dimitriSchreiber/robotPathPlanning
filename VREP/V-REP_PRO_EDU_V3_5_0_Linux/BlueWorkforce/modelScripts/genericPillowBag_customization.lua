...
function readInfo()
    local data=sim.readCustomDataBlock(model,'XYZ_PILLOWBAG_INFO')
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,'XYZ_PILLOWBAG_INFO',sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,'XYZ_PILLOWBAG_INFO','')
    end
end
...
function setColor(red,green,blue,spec)
    sim.setShapeColor(convex,nil,sim.colorcomponent_ambient_diffuse,{red,green,blue})
end

function getColor()
    local r,rgb=sim.getShapeColor(convex,nil,sim.colorcomponent_ambient_diffuse)
...
function updateModel()
    ...
    setObjectSize(model,w*0.7145,l*0.7145,h+0.002)
    setCuboidMassAndInertia(model,w*0.7145,l*0.7145,h,mass*0.5)

    setObjectSize(convex,w,l,h)
    setCuboidMassAndInertia(convex,w*0.7145,l*0.7145,h,mass*0.5)
    ...
    if sim.boolAnd32(bitCText,4)>0 then
        ...
        -- without texture
        sim.setShapeTexture(convex,-1,sim.texturemap_plane,4+8,{0.3,0.3})
    end
    
    -- Now the label:

    -- Remove the current labels:
    local objs=sim.getObjectsInTree(model,sim.object_shape_type,1+2)
    for i=1,#objs,1 do
        local h=objs[i]
...
function onCloseClicked()
    if sim.msgbox_return_yes==sim.msgBox(sim.msgbox_type_question,sim.msgbox_buttons_yesno,'Finalizing the pillow bag',"By closing this customization dialog you won't be able to customize the pillow bag anymore. Do you want to proceed?") then
        finalizeModel=true
    ...
    if (not ui) and simBWF.canOpenPropertyDialog() then
        ...
        <tabs id="77">
            ...
            <tab title="Colors/Texture" layout="form">
                    <checkbox text="Textured" on-change="texture_callback" id="888" />
                    <label text=""/>

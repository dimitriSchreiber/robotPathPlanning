...
handlePartAtLocation=function(h)
    ...
    sim.writeCustomDataBlock(s,simBWF.PART_TAG,sim.packTable(data))
        ...
        if enabled then
            if t-dwellStart>dwellTime then
                ...
                for i=1,#objects,1 do
                    ...
                    local dta=sim.readCustomDataBlock(objects[i],simBWF.PART_TAG)
                    dta=sim.unpackTable(dta)
                    dta['instanciated']=true
                    sim.writeCustomDataBlock(objects[i],simBWF.PART_TAG,sim.packTable(dta))
                end

...
function readInfo()
    local data=sim.readCustomDataBlock(model,simBWF.OLDOVERRIDE_TAG)
    if data then
...
function writeInfo(data)
    if data then
        sim.writeCustomDataBlock(model,simBWF.OLDOVERRIDE_TAG,sim.packTable(data))
    else
        sim.writeCustomDataBlock(model,simBWF.OLDOVERRIDE_TAG,'')
    end
end
...
if (sim_call_type==sim.customizationscriptcall_initialization) then
    ...
    local objs=sim.getObjectsWithTag(simBWF.OLDOVERRIDE_TAG,true)
    if #objs>1 then
        sim.removeObject(model)
        sim.removeObjectFromSelection(sim.handle_all)
        objs=sim.getObjectsWithTag(simBWF.OLDOVERRIDE_TAG,true)

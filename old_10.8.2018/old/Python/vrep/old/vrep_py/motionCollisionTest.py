...
if clientID!=-1:
    ...
    res,retInts,retFloats,retStrings,retBuffer=vrep.simxCallScriptFunction(clientID,'remoteApiCommandServer',vrep.sim_scripttype_childscript,'displayMessage',[],[],
        ['Computing and executing several linear paths, going through several waypoints. Collision detection is OFF.'],emptyBuff,vrep.simx_opmode_oneshot_wait)
    ...
    inFloats=robotInitialState+target2Pose
    ...
    if (res==0) and len(path)>0:
        ...
        #-------------------------------------
        #Now doing a path plan with collision checking
        #-------------------------------------
        ...
        inFloats=robotInitialState+target2Pose+approachVector #inFloats is a list. this appends the lists 
        print("inFloats: {}".format(inFloats)) 
        ...
        if (res==0) and len(path)>0:
            ...
            part1StateCnt=retInts[0] # number of states for normal part of path
            part2StateCnt=retInts[1] # number of states for linear part of path
            path1=path[:part1StateCnt*6] #6DOF arm I think for *6

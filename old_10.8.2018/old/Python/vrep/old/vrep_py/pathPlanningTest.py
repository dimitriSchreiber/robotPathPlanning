...
if clientID!=-1:
    ...
    res,robotHandle=vrep.simxGetObjectHandle(clientID,'IRB4600#',vrep.simx_opmode_oneshot_wait) #robot model
    ...
    inFloats=robotInitialState+target1Pose+approachVector #inFloats is a list. this appends the lists 
    ...
    print("found path length: {}".format(len(path)))
    ...
    if (res==0) and len(path)>0:
        ...
        if (res==0) and len(path)>0:
            ...
            res,retInts,retFloats,retStrings,retBuffer=vrep.simxCallScriptFunction(clientID,'remoteApiCommandServer',vrep.sim_scripttype_childscript,
                'visualizePath',[robotHandle,255,0,255],path,[],emptyBuff,vrep.simx_opmode_oneshot_wait)
            lineHandle=retInts[0]

            # Make the robot follow the path:
            res,retInts,retFloats,retStrings,retBuffer=vrep.simxCallScriptFunction(clientID,'remoteApiCommandServer',vrep.sim_scripttype_childscript,
                'runThroughPath',[robotHandle],path,[],emptyBuff,vrep.simx_opmode_oneshot_wait)

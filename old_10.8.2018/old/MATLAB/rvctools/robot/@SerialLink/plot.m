...
function opt = plot(robot, qq, varargin)
...
function h = create_robot(robot, opt)
    ...
    for L=1:N
        ...
        if opt.joints
            ...
            if links(L).isrevolute
                cyl('z', opt.jointscale*s, opt.jointdiam/2*s*[-1 1], opt.jointcolor, [], opt.jointalpha, 'Parent', h.link(L));
            else
        ...
        if robot.mdh
            ...
            if L < N
                ...
                t = transl(A);
                if t(1) ~= 0
                    cyl('x', s, [0 t(1)], opt.linkcolor, [], opt.linkalpha, 'Parent', h.link(L));
                end
                if t(2) ~= 0
                    cyl('y', s, [0 t(2)], opt.linkcolor, [t(1) 0 0], opt.linkalpha, 'Parent', h.link(L));
                end
                if t(3) ~= 0
                    cyl('z', s, [0 t(3)], opt.linkcolor, [t(1) t(2) 0], opt.linkalpha, 'Parent', h.link(L));
                end
            end
        else
            ...
            if L > 1
                ...
                if t(2) ~= 0
                    cyl('y', s, [s t(2)], opt.linkcolor, [t(1) 0 0], opt.linkalpha, 'Parent', h.link(L));
                end
                if t(3) ~= 0
                    cyl('z', s, [s t(3)], opt.linkcolor, [t(1) t(2) 0], opt.linkalpha, 'Parent', h.link(L));
                end
    ...
    t = transl(inv(tool));
    if t(1) ~= 0
        cyl('x', s, [0 t(1)], opt.toolcolor, [], opt.toolalpha, 'Parent', h.link(N+1));
    end
    if t(2) ~= 0
        cyl('y', s, [s t(2)], opt.toolcolor, [t(1) 0 0], opt.toolalpha, 'Parent', h.link(N+1));
    end
    if t(3) ~= 0
        cyl('z', s, [s t(3)], opt.toolcolor, [t(1) t(2) 0], opt.toolalpha, 'Parent', h.link(N+1));
    end
...
function cyl(ax, r, extent, color, offset, alpha, varargin)
    ...
    if isequal(color, 'none')
        ...
        surf(x,y,z, 'FaceColor', 'none', 'EdgeColor', 0.7*[1 1 1], 'EdgeLighting', 'none','FaceAlpha', alpha, varargin{:})
    else
        % walls of the cylinder
        surf(x,y,z, 'FaceColor', color, 'EdgeColor', 'none', 'FaceAlpha', alpha, varargin{:})
        % put the ends on
        patch(x', y', z', color, 'EdgeColor', 'none', 'FaceAlpha', alpha, varargin{:});
    end
end
